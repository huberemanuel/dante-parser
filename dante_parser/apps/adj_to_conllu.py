...
from dante_parser.data.conllu import extract_ids
    ...
    for i, line in enumerate(conllu_data):
        ...
        if r_sentence:
            ...
            if ori_tag != tag:
                ...
                if debug:
                    print(
                        f"Sentence {sent_id} replaced for token {token} from "
                        + f"{ori_tag} to {tag}"
                    )
            i_token += 1
...
def main():
    ...
    parser.add_argument(
        "input_xls", type=str, help="Input xls with adjucated sentences"
    )
