...
# reference: https://github.com/luciusssss/archybrid-parser/blob/master/archybrid.py
...
class Configuration:
        ...
        if sufficient_info:
            ...
            for word in sentence:
                if isinstance(word["id"], tuple):
                    # Ignoring multiword line.
                    continue
                child = int(word["id"])
                fa = int(word["head"])  #  previously "father"
    ...
    def oracle_update(self, transiton, shift_case):
        ...
        if transiton == SHIFT:
            if shift_case == 2:
                if (
                    self.father[b0] in self.stack.contents[:-1]
                    and b0 in self.rdeps[self.father[b0]]
                ):
    ...
    def transition_admissible(self, transition):
        ...
        if transition == SHIFT:
            ...
            return (
                ...
                and self.stack[-1] < self.buffer[0]
            )
    ...
    def calculate_cost(self):
        ...
        if not self.transition_admissible(LEFT_ARC):
            ...
            left_cost = len(self.rdeps[s0]) + int(
                self.father[s0] != b0 and s0 in self.rdeps[self.father[s0]]
            )

        if not self.transition_admissible(RIGHT_ARC):
            right_cost = 1
        else:
            right_cost = len(self.rdeps[s0]) + int(
                self.father[s0] != s1 and s0 in self.rdeps[self.father[s0]]
            )
        ...
        if not self.transition_admissible(SHIFT):
            ...
            len(
                [
                    item
                    for item in beta
                    if self.proj_order[item] < self.proj_order[b0] and item > b0
                ]
            )
            > 0
        ):
            ...
            shift_cost = len([d for d in self.rdeps[b0] if d in self.stack]) + int(
                s0 != None
                and self.father[b0] in self.stack.contents[:-1]
                and b0 in self.rdeps[self.father[b0]]
            )
            shift_case = 2
